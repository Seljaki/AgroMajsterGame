package com.seljaki.AgroMajsterGame.assets;

public class AssetPaths {
    public static final String GAMEPLAY = "images/assetsAtlas.atlas";
    public static final String UI_SKIN = "ui/skin/flat-earth-ui.json";
    public static final String GUN_SHOT = "sounds/gunShot.wav";
    public static final String EMPTY_GUN_SHOT = "sounds/emptyGunShot.wav";
    public static final String RELOAD_SOUND = "sounds/reload.wav";
    public static final String SELECT_SOUND = "sounds/select.wav";
    public static final String GAME_OVER_SOUND = "sounds/gameOver.wav";
    public static final String SCORE_SOUND = "sounds/score.wav";
    public static final String GAME_FONT = "ui/gameFont.fnt";

    public static final String SKIN = "ui/uiskin.json";

    public static final String MOLE_SQUEAK_SOUND = "sounds/moleSqueak.wav";
    public static final String WHACK_A_MOLE_MUSIC = "music/whack_a_mole_music.ogg";
    public static final String PARTICLE_EFFECT_MOLE_BLOOD = "particles/mole_blood.p";

    private AssetPaths() {
    }
}
